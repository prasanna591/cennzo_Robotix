  ...
  return (
    ...
    >
      <motion.span
        ...
        className={`hairline-spectrum relative h-px ${centered ? "min-w-16 flex-1" : "w-8"}`}
      />
      ...
      <motion.span
        ...
        whileInView="visible"
        viewport={viewportOnce}
        className={`hairline-spectrum relative h-px ${centered ? "min-w-16 flex-1" : "min-w-12 flex-1"}`}
      >
        <span className="absolute right-0 top-1/2 block h-1 w-1 -translate-y-1/2 rounded-full bg-teal" />
      </motion.span>
    </p>
  );
}
